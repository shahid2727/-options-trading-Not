...
def market_levels(t):
    try:
        ...
        support=float(l.tail(20).min()); resistance=float(h.tail(20).max())
    ...
    for symbol in symbols:
        try:
            ...
            for exp in t.options:
                dte=(datetime.fromisoformat(exp).replace(tzinfo=timezone.utc)-datetime.now(timezone.utc)).days
                ...
                for side,df in [('C',chain.calls),('P',chain.puts)]:
                    for _,row in df.iterrows():
                        ...
                        out.append({
                            'symbol':symbol,'contract':f"{num(r.get('strike')):g}{side}",'expiration':exp,'dte':dte,
